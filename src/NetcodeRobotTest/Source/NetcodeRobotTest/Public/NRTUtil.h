...
extern TMap<void*, InternalProcessEventCallback> ActiveRobotProcessEventCallbacks;
#endif
...
static bool HandleProcessEventCallback(AActor* Actor, UFunction* Function, void* Parameters)
{
    ...
    for (auto It = ActiveRobotProcessEventCallbacks.CreateConstIterator(); It; ++It)
    {
...
static void AddProcessEventCallback(void* HookOrigin, InternalProcessEventCallback InCallback)
{
    if (HookOrigin != NULL)
    {
        if (ActiveRobotProcessEventCallbacks.Num() == 0)
        {
            AActor::ProcessEventDelegate.BindStatic(&HandleProcessEventCallback);
        }

        ActiveRobotProcessEventCallbacks.Add(HookOrigin, InCallback);
    }
    else
    {
...
static void RemoveProcessEventCallback(void* HookOrigin, InternalProcessEventCallback InCallback)
{
    if (HookOrigin != NULL)
    {
        ActiveRobotProcessEventCallbacks.Remove(HookOrigin);
...
struct NRTUtil
{
    ...
    /**
     * Overridden parse function, for supporting escaped quotes, e.g: -RobotTestServerParms="-LogCmds=\"LogNet all\""
     */
